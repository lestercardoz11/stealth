...
export async function POST(req: Request) {
  try {
    ...
    if (documentIds && documentIds.length > 0) {
      try {
        console.log(
          'Processing selected documents for context...',
          documentIds
        );
        ...
        if (docError) {
          ...
          console.log(
            `Retrieved ${selectedDocuments.length} selected documents:`,
          ...
          if (documentsWithContent.length > 0) {
            ...
            console.log(
              `Assembled context from ${documentsWithContent.length} documents with content`
            );
            ...
            context = selectedDocuments
              .map(
                (doc) =>
                  `Document "${doc.title}" was selected but contains no readable content.`
              )
              .join('\n');
          }
          ...
          try {
            ...
            if (relevantChunks && relevantChunks.length > 0) {
              ...
              console.log(
                `Found ${relevantChunks.length} relevant chunks for source attribution`
              );
            } else {
              ...
              console.log(
                `Created sources from ${sources.length} selected documents`
              );
            }
          } catch (searchError) {
